                ...
                if MODEL_TYPE == 'fusion':
                    ...
                    if USE_GRAPH:
                        combined_output = self.fusion_module(
                            exemplar_tensor=exemplar_output,
                            timeline_tensor=timeline_output,
                            graph_tensor=graph_output,
                            lm_tensor=hidden_states
                        )
                        ...
                        combined_output = self.fusion_module(
                            exemplar_tensor=exemplar_output,
                            timeline_tensor=timeline_output,
                            evidence_tensor=evidence_output,
                            lm_tensor=hidden_states
                        )
                        hidden_states = combined_output
                    ...
                    if USE_GRAPH:
                        hidden_states = torch.cat([exemplar_output, timeline_output, graph_output, hidden_states], dim=1)
                    
                    elif USE_EVIDENCE:
                        hidden_states = torch.cat([exemplar_output, timeline_output, evidence_output, hidden_states], dim=1)
                    else:
                        hidden_states = torch.cat([exemplar_output, timeline_output, hidden_states], dim=1)
...
class HateBertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        if DML_MIXER:
            print("\nUsing DML_MIXER\n")
            self.exemplar_dml_mixer = DMLMixer(model_dim=config.hidden_size)
            self.timeline_dml_mixer = DMLMixer(model_dim=config.hidden_size)
